...
fn get_pair_frequencies(data: &[usize]) -> HashMap<[usize; 2], usize> {
    let mut pair_frequencies: HashMap<usize, usize> = HashMap::new();
    for i in 0..data.len() - 1 {
        *pair_frequencies.entry(data[i] + data[i + 1] << 32).or_insert(0) += 1;
    }
    pair_frequencies.into_iter().map(|(key, value)| {
        let a = key & 0xFFFFFFFF;
        let b = key >> 32;
        ([a, b], value)
    }).collect()
}
